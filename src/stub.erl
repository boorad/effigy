...
stub(Module, Function, Fun, Times) when is_function(Fun) ->
  gen_server:start({local, name(Module, Function)}, ?MODULE,
                   [Module, Function, Fun, Times], []).
...
init([Module, Function, Fun, Times]) ->
  case code:get_object_code(Module) of
    {Module, Bin, Filename} ->
      ...
      {ok, #state{module=Module,old_code={Module,Bin,Filename},times=Times,
                  stub=Fun}};
...
stub_function(Module, Function, Arity) ->
  ...
  case compile:forms(StubbedForms, [binary]) of
    {ok, Module, Binary} -> code:load_binary(Module, atom_to_list(Module) ++
                                             ".erl", Binary);
    Other -> Other
  end.
...
replace_function(_Module, _Function, _Arity, [], Acc) ->
  lists:reverse(Acc);
replace_function(Module, Function, Arity, [{function, Line, Function, Arity,
                                            _Clauses}|Forms], Acc) ->
  lists:reverse(Acc) ++ [{function, Line, Function, Arity, [
    {clause,
      ...
      [],
      generate_expression(stub,proxy_call,Module,name(Module,Function),Arity)}]}]
        ++ Forms;
